...
- scanner/runtime comparators should follow the same rule for future `Control Panel` and `Execution Logs` surfaces; equality must compare target cohorts and run cohorts semantically instead of only checking array lengths or the latest run id
...
- for `Signal Bot` specifically, the shared selector should expose only `signalMemory + activeWatchlistCoins`; non-active watchlist edits must not wake the page just because the full watchlist collection changed
- template-facing `signals + bots` pages should also share one derived domain read-model seam on top of that narrow selector; `Signal Bot`, `Bot Settings`, `Control Panel` and future bot pages must not each rebuild their own ranked feed pipeline with slightly different local derivations
- `Market Core` and `Signal Core` should also stay explicit shared seams above the planes; market context can evolve independently from feed taxonomy, and pages should consume those seams instead of recomposing `market plane + signal memory + watchlist` ad hoc
- bot-facing pages should hydrate their registry from one shared persistence seam instead of keeping a page-local bot list; `Bot Settings`, quick edit, selected bot context and the full bot workspace must all read/write through the same shared bot registry runtime
