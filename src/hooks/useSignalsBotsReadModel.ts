...
>(decisions: TDecision[]): BotExecutionIntentSummary {
  ...
  return {
    ...
    dispatchedCount: ranked.filter((entry) => entry.laneStatus === "previewed" || entry.laneStatus === "execution-submitted").length,
    previewedCount: ranked.filter((entry) => entry.laneStatus === "previewed").length,
