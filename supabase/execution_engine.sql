...
create table if not exists public.execution_profiles (
  ...
  max_daily_auto_executions integer not null default 3,
  cooldown_after_losses integer not null default 2,
...
alter table public.execution_profiles
  ...
  add column if not exists max_daily_auto_executions integer not null default 3,
  add column if not exists cooldown_after_losses integer not null default 2,
...
alter table public.execution_orders
  ...
  add column if not exists protection_status text not null default 'none',
  add column if not exists signal_outcome_status text,
  add column if not exists realized_pnl numeric not null default 0,
  add column if not exists linked_order_ids jsonb not null default '{}'::jsonb,
